...
| **Mudança no agente** | loop, system prompt, dispatch em `agent.py` | Teste em `tests/test_agent.py`; se mexer no system prompt, preservar a regra do `isTrusted` (inegociável 2); rodar a suíte completa `pytest tests/` local (o job `test` do CI roda `test_tools` + `test_agent`; `test_smoke` fica fora por depender de httpbin.org) |
